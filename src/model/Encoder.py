...
class CompleteSubwordEncoder(object):
	...
	def encode(self, w, o, l, include_output = False, include_lemmas = True, include_ngrams = True, sum_embeddings=True, noramlize=False):
		...
		#outputs embeddings
		output_embedding = dy.vecInput(EMBEDDING_SIZE)
		if o==[]: o=["<unk>"]
		for out_token in o:
			out_token_encoded = self.OUTPUT2IND[out_token] if out_token in self.OUTPUT2IND else self.OUTPUT2IND["<unk>"]
			out_embedding = dy.lookup(self.E_output, out_token_encoded)
			if include_output: output_embedding = output_embedding + out_embedding
		
		if noramlize: output_embedding/=len(o)

		#lemmas embeddings
		lemmas_embedding = dy.vecInput(EMBEDDING_SIZE)
		if l==[]: l=["<unk>"]
		for lemma in l:
			l_encoded = self.LEMMA2I[lemma] if lemma in self.LEMMA2I else self.LEMMA2I["<unk>"]
			l_embedding = dy.lookup(self.E_lemmas, l_encoded)
			if include_lemmas: lemmas_embedding = lemmas_embedding + l_embedding
